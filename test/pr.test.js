...
describe('PR gate CLI and metadata', () => {
  ...
  it('publishes PR gate commands in schema metadata', () => {
    ...
    assert.equal(body.supportsDryRun, false);
    assert.equal(body.flagDetails.find(flag => flag.name === '--json').type, 'boolean');
    assert.equal(gate.mutates, true);
    assert.deepEqual(gate.mutationTargets, ['github']);
    assert.equal(gate.supportsJson, true);
    assert.equal(gate.supportsDryRun, true);
    assert.equal(gate.flagDetails.find(flag => flag.name === '--dry-run').type, 'boolean');
  });
});
